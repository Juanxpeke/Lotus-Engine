/*
  Implementation copied from MeineKraft source code (https://github.com/lingtorp/MeineKraft)
*/
...
namespace Lotus
{
  ...
  struct Vec4
  {
    ...
    {
      switch (index)
      { // Should be a jump table when optimized
        case 0:
        return x;
        case 1:
        return y;
  ...
  struct Vec3
  {
    ...
    constexpr inline T sqrLength() const { return x * x + y * y + z * z; }
  ...
  public:
    ...
    constexpr inline Mat4<T> setTranslation(const Vec3<T>& vec) const
    {
      Mat4<T> matrix;
    ...
    constexpr inline Vec3<T> getTranslation() const
    {
